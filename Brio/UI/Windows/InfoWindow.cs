    ...
    public override void Draw()
    {
        ...
        using (var textGroup = ImRaii.Group())
        {
            if (textGroup.Success)
            {
                ...
                text += "\n\n";
                text += "Brio is a suite of tools to enhance your GPosing experience, originally developed by Asgard now maintained by Kisa Valentine.";
                text += "\n\n";
        ...
        using (var buttonGroup = ImRaii.Group())
        {
            if (buttonGroup.Success)
            {
                ...
                if (ImGui.Button("Report Issue", buttonSize))
                    Process.Start(new ProcessStartInfo { FileName = "https://github.com/Valentine-Tools/Brio/issues", UseShellExecute = true });
                ImGui.PopStyleColor();

                ImGui.PushStyleColor(ImGuiCol.Button, new Vector4(110, 84, 148, 255) / 255);
                if (ImGui.Button("GitHub Repository", buttonSize))
                    Process.Start(new ProcessStartInfo { FileName = "https://github.com/Valentine-Tools/Brio", UseShellExecute = true });
                ImGui.PopStyleColor();

                ImGui.PushStyleColor(ImGuiCol.Button, new Vector4(0, 70, 0, 255) / 255);
                if (ImGui.Button("Licenses", buttonSize))
                    Process.Start(new ProcessStartInfo { FileName = "https://github.com/Valentine-Tools/Brio/blob/main/Acknowledgements.md", UseShellExecute = true });
                ImGui.PopStyleColor();
